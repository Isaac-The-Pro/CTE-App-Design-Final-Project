...
#Minesweeper
#clear tiles by left clicking on them.
#Tiles next to one or more mines will have the number of mines next to them
#right click to place flags on mines
#if you click on a mine, you lose
#if you clear all the tiles that don't have mines, you win
    ...
    def __init__(self, minesX, minesY, tileWidth):
        ...
        for row in self.tiles:
            for tile in row:
                ...
                self.canvas.tag_raise(tile.flag)
...
class Tile:
    def __init__(self, x, y, width, canvas):
       ...
       self.touchesMines = 0
       self.clicked=False
       self.flag = canvas.create_oval(self.realX+width/4, self.realY+width/4, self.realX+3*width/4, self.realY+3*width/4, fill="red", outline="black", state='hidden')
       self.hasFlag = False
